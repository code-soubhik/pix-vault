...
const Login = () => {
  ...
  const { login, refreshSession, isAuthenticated } = useAuth()
  const { push } = useToast()
  ...
  const handleSubmit = async (e) => {
    e.preventDefault()
    if (!email || !password) {
      push({ type: 'error', message: 'Email and password are required' })
      return
    }
    setSubmitting(true)
    try {
    ...
    <div className="min-h-screen flex items-center justify-center bg-gradient-to-b from-[#0b1220] to-[#0b0f17] p-6">
      <div className="w-full max-w-md bg-white/5 border border-white/10 rounded-2xl p-8 text-white shadow-2xl shadow-black/30">
        ...
        <p className="mt-4 text-center text-sm text-white/70">
          No account? <Link to="/signup" className="text-blue-400 hover:underline">Create one</Link>
        </p>
      </div>
    </div>
  )
